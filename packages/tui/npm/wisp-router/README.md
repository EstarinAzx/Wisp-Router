...
**2.2.1:** signed-in Codex desktop models retain native effort choices, context metadata, instructions and native sign-in even when shared Wisp routes override the same name. Use a distinct Alias for external desktop models. Other Wisp clients keep their existing routes. `wisp codex-desktop enable|status|refresh|disable` manages reversible setup; repeated enable repairs older reduced catalogs. [Setup, limits and rollback](https://github.com/EstarinAzx/Wisp-Router/blob/main/docs/codex-desktop.md).

Update the setup command and serving Bridge together: desktop protocol 3 requires terminal 2.2.1 or companion VSIX 1.14.1. Restart Codex after catalog changes; reload VS Code after updating its companion. Native/account refresh still requires disable/discover/enable. Antigravity Aliases remain excluded. Your context/compaction preferences are preserved, but Codex's native limits still apply; this release does not unlock a full 1M usable window. Slot is unchanged.
