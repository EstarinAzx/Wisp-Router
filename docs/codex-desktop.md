...
Exact Wisp Aliases and existing exact Codex model overrides use their pinned Provider/model. An Alias wins a native-name collision. Unoverridden native IDs from the saved native view pass only to the fixed native Codex endpoint with their body intact. Unknown IDs fail locally; the signed path has no Active Provider or family fallback. Route edits apply on the next request; refresh updates picker labels and capabilities.

**Antigravity Targets are excluded from signed desktop choices** because their wire cannot preserve the desktop's positioned developer-instruction semantics. Wisp does not consolidate or reorder those instructions. Enable/refresh/status report omitted aliases and native-name overrides explicitly; a missing native-name choice may reflect its existing incompatible Wisp override, not lost native support. Unset/change that override if you want the native route. Explicit signed requests to excluded targets fail before credential lookup or upstream work. Existing Wisp routes and other clients keep their previous behavior.

External routes do not offer provider-hosted web search. Codex may still attach a prospective web_search declaration despite the conservative catalog flag; Wisp omits only that unused top-level declaration for absent/automatic tool choice. Ordinary function/custom/namespace tools named web_search are preserved. Forced hosted choices, executed search history, unsupported hosted options/includes, opaque reasoning/compaction and other unsupported shapes still fail. Global native search settings are unchanged; native passthrough retains native search/tools.
