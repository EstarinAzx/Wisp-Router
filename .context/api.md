...
- **`POST /v1/chat/completions`:** `parseOpenAiChatRequest` (untrusted body → **400** on bad JSON or no turns) →
  ...
  (`RoutingMap` in extension globalState `wisp.routingMap`) + panel model are read **live per request** — a
