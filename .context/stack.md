...
## Testing
- `vitest` `^4.1` (devDep of `@wisp/core`) — unit-test runner for the **vscode-free** pure logic (`packages/core/src/*.test.ts`, 352 tests). Run `bun run test` at root. No `@vscode/test-electron`: the tested functions are pure, so no Extension Development Host is needed. Core's `tsconfig.json` excludes `src/**/*.test.ts` from typecheck (mirrors the old build exclusion).
