...
- `packages/tui/` — the **TUI face** (#60): npm **`wisp-router` 0.1.0**, bin `wisp` (`claude-wisp` bin lands with #64). opentui 0.4.3 (`@opentui/core` + `@opentui/react`) + React 19 on Bun; no build step — `bun run dev` runs `src/index.tsx` (entry: `wisp serve` argv branch → headless `serve.ts`, else boot the renderer) + `src/app.tsx` (splash, slash palette, `/providers` `/key` `/model` `/signin` `/signout` `/effort` `/test` `/bridge` `/quit`; the OAuth commands drive core's `CodexAuth`/`AnthropicAuth`, and `/test` (#62) streams one canned prompt via the exported `streamTestReply`). Since #63 the TUI also **hosts the Bridge**: `src/store.ts` (shared `~/.wisp` handle + OAuth managers), `src/bridge.ts` (BridgeDeps wiring — twin of the extension's), `src/serve.ts` (`wisp serve`: the process without a face — no daemon, Ctrl+C stops). Both faces share the Bridge port + secret; a second host fails loud (EADDRINUSE), no port-hop.
...
- TUI: `cd packages/tui; bun run dev` (writes real `~/.wisp`; set `WISP_HOME` to sandbox). Headless Bridge: `bun src/index.tsx serve`.
