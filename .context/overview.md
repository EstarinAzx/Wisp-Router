...
- `packages/tui/` — the **TUI face** (#60): npm **`wisp-router` 0.1.0**, bin `wisp` (`claude-wisp` bin lands with #64). opentui 0.4.3 (`@opentui/core` + `@opentui/react`) + React 19 on Bun; no build step — `bun run dev` runs `src/index.tsx` (boot) + `src/app.tsx` (splash, slash palette, `/providers` `/key` `/model` `/quit` over `WispHome`).
...
- Test: `bun run test` (root → 344 Vitest tests in `packages/core/src/*.test.ts`; no Electron host).
- TUI: `cd packages/tui; bun run dev` (writes real `~/.wisp`; set `WISP_HOME` to sandbox).
