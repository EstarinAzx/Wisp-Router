...
- `packages/tui/` — the **TUI face** (#60): npm **`wisp-router` 0.1.0**, bin `wisp` (`claude-wisp` bin lands with #64). opentui 0.4.3 (`@opentui/core` + `@opentui/react`) + React 19 on Bun; no build step — `bun run dev` runs `src/index.tsx` (boot) + `src/app.tsx` (splash, slash palette, `/providers` `/key` `/model` `/signin` `/signout` `/effort` `/test` `/quit` over `WispHome`; the OAuth commands drive core's `CodexAuth`/`AnthropicAuth` with a spawn-based browser opener, and `/test` (#62) streams one canned prompt through a Provider or Alias via the exported `streamTestReply`).
...
- Test: `bun run test` (root → 352 Vitest tests in `packages/core/src/*.test.ts`; no Electron host).
