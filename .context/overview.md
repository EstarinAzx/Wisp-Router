...
Since source commit `e23f103` (#209–211 complete), the native Codex `/v1/responses` subset and `codex-wisp` source/compiled/npm launchers are prepared for terminal 2.1.4. See `docs/codex-wisp.md` for invocation, routing, supported content/tools and retained checks. Windows local artifacts and independent review are complete; publication and global installation are pending. Installed Wisp 2.1.3 and extension 1.13.6 are unchanged. [[active-work]] points to artifact hashes and coordinator verification.
...
- Test: `bun run test` (root → core Vitest suite; no Electron host). Terminal tests: `bun test packages/tui/tests/`. Latest verification counts and native/package evidence are in [[active-work]].
